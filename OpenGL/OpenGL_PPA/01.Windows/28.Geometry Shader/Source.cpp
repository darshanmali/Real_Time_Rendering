...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreIntance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        Appname,
        TEXT("OpenGL Programmable Pipeline : Geometric Shader "),
...
void Initialize()
{
    ...
    const GLchar* GeometyShaderSourceCode =
        "#version 450 core\n" \
        "layout(triangles)in;\n" \
    ...
    glShaderSource(GeometryShaderObject, 1, (const GLchar**)&GeometyShaderSourceCode, NULL);
    ...
    if (iShaderCompileStatus == GL_FALSE)
    {
        ...
        if (iInfoLogLength > 0)
        {
            ...
            if (szInfoLog != NULL)
            {
                ...
                fprintf_s(gpFile_DM, "Geometry Shader Compilation Log : %s \n", szInfoLog);
                free(szInfoLog);
                unInitialize();
                exit(0);
            }
