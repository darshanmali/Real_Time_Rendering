...
GLuint gVao_Quade;
GLuint gVbo_Quade_Position;
GLuint gVbo_Quade_Color;
...
/* ******************************************OpenGL PP ******************************************************* */
	...
	XStoreName ( gpDisplay, gWindow, "Darshan's OpenGL PP :Multi Color Triangle and Quade XWindows");
...
/*New for traingle */
	...
	/* Quade Began */
    glGenVertexArrays(1, &gVao_Quade);
    glBindVertexArray(gVao_Quade);

    glGenBuffers(1, &gVbo_Quade_Position);
    glBindBuffer(GL_ARRAY_BUFFER, gVbo_Quade_Position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(quadeVertices), quadeVertices, GL_STATIC_DRAW);
    glVertexAttribPointer(DVM_ATTTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);

    glEnableVertexAttribArray(DVM_ATTTRIBUTE_POSITION);
    glBindBuffer(GL_ARRAY_BUFFER, 0);

	glGenBuffers(1, &gVbo_Quade_Color);
    glBindBuffer(GL_ARRAY_BUFFER, gVbo_Quade_Color);
    glBufferData(GL_ARRAY_BUFFER, sizeof(quadeColor), quadeColor, GL_STATIC_DRAW);
    glVertexAttribPointer(DVM_ATTTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(DVM_ATTTRIBUTE_COLOR);
...
void uninitialize(void)
{
	...
	if (gVao_Quade)
    {
        glDeleteVertexArrays(1, &gVao_Quade);
        gVao_Quade = 0;
    }

    if (gVbo_Quade_Position)
    {
        glDeleteBuffers(1, &gVbo_Quade_Position);
        gVbo_Quade_Position = 0;
    }

	if (gVbo_Quade_Color)
    {
        glDeleteBuffers(1, &gVbo_Quade_Color);
        gVbo_Quade_Color = 0;
    }
