...
package com.darshan_vilas_mali.rotation_2d;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener, OnDoubleTapListener{
	...
	private int[] Square_vao = new int[1];
	private int[] Square_vbo_Position = new int[1];
	private int[] Square_vbo_Color = new int[1];

	private int[] Triangle_vao = new int[1];
	private int[] Triangle_vbo_Position = new int[1];
	private int[] Triangle_vbo_Color = new int[1];

	private int mvpUniform;

	private float angle_T = 0.0f;
	private float angle_S = 0.0f;
	...
	/*Square */
		...
		//Triangle Began
		GLES32.glGenVertexArrays(1, Triangle_vao, 0);
		GLES32.glBindVertexArray(Triangle_vao[0]);

		// THIS is Vertex VBO
		GLES32.glGenBuffers(1, Triangle_vbo_Position, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, Triangle_vbo_Position[0]);
		...
		/* This  Color VBO */
		GLES32.glGenBuffers(1, Triangle_vbo_Color, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, Triangle_vbo_Color[0]);
		...
		//*********************Square Began*************************
		GLES32.glGenVertexArrays(1, Square_vao, 0);
		GLES32.glBindVertexArray(Square_vao[0]);

		// THIS is Vertex VBO
		GLES32.glGenBuffers(1, Square_vbo_Position, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, Square_vbo_Position[0]);
		...
		/* This  Color VBO */
		GLES32.glGenBuffers(1, Square_vbo_Color, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, Square_vbo_Color[0]);
		byteBuffer = ByteBuffer.allocateDirect(SquareColor.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		vertexBuffer = byteBuffer.asFloatBuffer();
		vertexBuffer.put(SquareColor);
		vertexBuffer.position(0);
	...
	private void display()
	{
		...
		Matrix.translateM(translateMatrix,0, -1.5f, 0.0f,-4.0f);

		Matrix.rotateM(RotationMatrix,0, angle_T,0.0f, 1.0f, 0.0f);
		...
		Matrix.translateM(translateMatrix,0, 1.5f, 0.0f, -4.0f);
		Matrix.rotateM(RotationMatrix,0, angle_S,0.0f, 1.0f, 0.0f);
	...
	private void UnInitialized()
	{
		if(Square_vao[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, Square_vao, 0);
			Square_vao[0] = 0;
		}
		if(Square_vbo_Position[0] != 0)
		{
			GLES32.glDeleteBuffers(1, Square_vbo_Position, 0);
			Square_vbo_Position[0] = 0;
		}
		if(Square_vbo_Color[0] != 0)
		{
			GLES32.glDeleteBuffers(1, Square_vbo_Color, 0);
			Square_vbo_Color[0] = 0;
		}

		if(Triangle_vao[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, Triangle_vao, 0);
			Triangle_vao[0] = 0;
		}
		if(Triangle_vbo_Position[0] != 0)
		{
			GLES32.glDeleteBuffers(1, Triangle_vbo_Position, 0);
			Triangle_vbo_Position[0] = 0;
		}
		if(Triangle_vbo_Color[0] != 0)
		{
			GLES32.glDeleteBuffers(1, Triangle_vbo_Color, 0);
			Triangle_vbo_Color[0] = 0;
		}
