...
ID3D11GeometryShader* gpID3D11GeomertryShader = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        szClassname,
        TEXT("Darshan's : Direct3D11 BW Geometry Shader...!"),
...
HRESULT Initialize()
{
    ...
    const char* vertexShaderSourceCode =
        ...
        "VertexOutPut main(float4 pos : POSITION) : POSITION"\
        "{"\
        "VertexOutPut outPut;"\
    ...
    const char* GeometryShaderSourceCode =
        ...
        "void main(triangle VertexOutPut input_array[3], inout TriangleStream < Geometry_OutPut > trianglestream){"\
        "Geometry_OutPut output;"\

        "for(int vertex = 0; vertex < 3; vertex++){\n" \

        "output.position = mul(worldViewProjectionMatrix, (input_array[vertex].position + float4(0.0, 1.0, 0.0, 0.0)));\n" \
        "trianglestream.Append(output);\n" \
        "output.position = mul(worldViewProjectionMatrix, (input_array[vertex].position + float4(1.0, -1.0, 0.0, 0.0)));\n" \
        "trianglestream.Append(output);\n" \
        "output.position = mul(worldViewProjectionMatrix, (input_array[vertex].position + float4(-1.0, -1.0, 0.0, 0.0)));\n" \
        "trianglestream.Append(output);\n" \
    ...
    hr = D3DCompile(GeometryShaderSourceCode,//code
        lstrlenA(GeometryShaderSourceCode) + 1,//Length
        "GS",//VErtexShader Compiler Identifier
        NULL,//
        D3D_COMPILE_STANDARD_FILE_INCLUDE,//
        "main",//
        "gs_5_0",//version 
    ...
    if (FAILED(hr))
    {
        ...
        {
            fopen_s(&gpFile_DM, gszLogFileName, "a+");
            fprintf_s(gpFile_DM, "D3D11Compile() Failed. For Geometry  Shader : %s.\n", (char *)pID3DBlob_Error->GetBufferPointer());
            fclose(gpFile_DM);
            pID3DBlob_Error->Release();
            pID3DBlob_Error = NULL;
            return(hr);
    ...
    hr = gpID3D11Device->CreateGeometryShader(pID3DBlob_GeometryShaderCode->GetBufferPointer(),
        ...
        NULL,
        &gpID3D11GeomertryShader);
    if (FAILED(hr))
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "D3D11Devices::CreateGeometryShader() Failed.\n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "D3D11Devices::CreateGeomtryShader() Succeedded.\n");
        fclose(gpFile_DM);
    }

    gpID3D11DeviceContext->GSSetShader(gpID3D11GeomertryShader, 0, 0);
...
void unInitialize()
{
    ...
    if (gpID3D11GeomertryShader)
    {
        gpID3D11GeomertryShader->Release();
        gpID3D11GeomertryShader = NULL;
    }
