...
ID3D11Buffer* gpID3D11_Cube_Buffer_VertextBuffer_Position = NULL;
ID3D11Buffer* gpID3D11_Cube_Buffer_VertextBuffer_Texture = NULL;
...
unsigned char Check_Image[CHECK_IMAGE_HIGHT][CHECK_IMAGE_WIDTH][4];
...
//Texture Interface
ID3D11ShaderResourceView* gpid3d11_ShaderResourceView_Texture = NULL;
ID3D11SamplerState* gpid3d11_SamplerState_Texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        szClassname,
        TEXT("Darshan's : Direct X 3D Checker Board...!"),
...
HRESULT Initialize()
{
    ...
    void unInitialize(void);
    HRESULT Resize(int, int);
    HRESULT Load_D3D_Texture(const wchar_t*, ID3D11ShaderResourceView **);
    ...
    /*
        texture() from Opengl is similar to "Texture2D" in DirectX, it is also a structure HSL
    */
    ...
    */
    ...
    inputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;//glPixelStorei unpackAlling  
    ...
    //this Cube_vertices are taken as clock vise but openGL use anti clock vise
    float Cube_vertices[] =
    {
        // SIDE 1 ( TOP )
        -1.0f, +1.0f, +1.0f,
    ...
    /*
       These are the Coordinate system of 
       Opengl/DirectX and Texture in DetecX and texture in Opengl 
            X         ==           U         ==        S 
            Y         ==           V         ==        T
            Z         ==           W         ==        R  
    ...
    */
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
        NULL,
        &gpID3D11_Cube_Buffer_VertextBuffer_Position );
    ...
    //Copy Cube_vertices into above created buffer
    D3D11_MAPPED_SUBRESOURCE mappedSubresource;
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));

    gpID3D11DeviceContext->Map(gpID3D11_Cube_Buffer_VertextBuffer_Position ,
        0,
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11_Cube_Buffer_VertextBuffer_Position , 0);

    //Pyramid_Color buffer Creation
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(Cube_TexCoord);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
        NULL,
        &gpID3D11_Cube_Buffer_VertextBuffer_Texture);

    if (FAILED(hr))
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "ID3D11Device::CreateBuffer() Failed for Cube_vertices Buffer. \n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "ID3D11Device::CreateBuffer() Succeeded for Cube_vertices Buffer. \n");
        fclose(gpFile_DM);
    }

    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));

    gpID3D11DeviceContext->Map(gpID3D11_Cube_Buffer_VertextBuffer_Texture,
        0,
        D3D11_MAP_WRITE_DISCARD,
        0,
        &mappedSubresource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11_Cube_Buffer_VertextBuffer_Texture, 0);
    ...
    /*
        Texture cdreation call
    */
    hr = Load_D3D_Texture(L"Vijay_Kundali.bmp", &gpid3d11_ShaderResourceView_Texture);//L == Literal before string (wide char compliand)
    
    if (FAILED(hr))
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "gpID3D11Device->Load_D3D_Texture() Failed For Loading Texture.\n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "gpID3D11Device->Load_D3D_Texture() succed For Loading Texture.\n");
        fclose(gpFile_DM);
    }
        
    D3D11_SAMPLER_DESC d3d11SamplerDesc;
    ZeroMemory(&d3d11SamplerDesc, sizeof(D3D11_SAMPLER_DESC));

    d3d11SamplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
    d3d11SamplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;// COVERING THE MODEL == WRAP
    d3d11SamplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
    d3d11SamplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;

    hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpid3d11_SamplerState_Texture);

    if (FAILED(hr))
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "gpID3D11Device->CreateSamplerState() Failed For Creating Sampler State.\n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "gpID3D11Device->Load_D3D_Texture() succed For Creating Sampler State.\n");
        fclose(gpFile_DM);
    }
...
HRESULT Load_D3D_Texture(const wchar_t *textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
    HRESULT hr;
    ...
    D3D11_TEXTURE2D_DESC  d3d11_Texture2D_Desk_Local;
    ZeroMemory(&d3d11_Texture2D_Desk_Local, sizeof(D3D11_TEXTURE2D_DESC));

    d3d11_Texture2D_Desk_Local.Width = (UINT)CHECK_IMAGE_WIDTH;
    d3d11_Texture2D_Desk_Local.Height = (UINT)CHECK_IMAGE_HIGHT;
    d3d11_Texture2D_Desk_Local.Format = DXGI_FORMAT_R8G8B8A8_UNORM;//Depth is 32 bit
    d3d11_Texture2D_Desk_Local.Usage = D3D11_USAGE_DEFAULT;
    d3d11_Texture2D_Desk_Local.BindFlags = D3D11_BIND_SHADER_RESOURCE;
    d3d11_Texture2D_Desk_Local.SampleDesc.Count = 1; // THIS CAN BE 1 TO 4 ( WHICH DEFINES QUALITY )
    d3d11_Texture2D_Desk_Local.SampleDesc.Quality = 0; // 0 TELLS DEFAULT QUALITY.
    d3d11_Texture2D_Desk_Local.ArraySize = 1;
    d3d11_Texture2D_Desk_Local.MipLevels = 1; // 1 == DEFAULT MIP MAP LEVEL
    d3d11_Texture2D_Desk_Local.CPUAccessFlags = 0; //0 == Default
    d3d11_Texture2D_Desk_Local.MiscFlags = 0;

    D3D11_SUBRESOURCE_DATA Resource;
    ZeroMemory(&Resource, sizeof(D3D11_SUBRESOURCE_DATA));

    Resource.pSysMem = Check_Image;
    Resource.SysMemPitch = CHECK_IMAGE_WIDTH * 4;

    ID3D11Texture2D* Texture_2D_Buffer = NULL;


    /*2nd Parrmeter : d3d11Sub_Resource_Data = This is used if you have some data right now else it has to be null.*/
    hr = gpID3D11Device->CreateTexture2D(&d3d11_Texture2D_Desk_Local, &Resource, &Texture_2D_Buffer);
    ...
    hr = gpID3D11Device->CreateShaderResourceView(Texture_2D_Buffer, NULL, ppID3D11ShaderResourceView);

    /*
        DirectX::createWICTextureFromMemory() : This fun will create texture from memory with resource file help.
    */
    return(hr);
}
...
void MakeCheckImage(void)
{
    ...
    for (i = 0; i < CHECK_IMAGE_HIGHT; i++)
    {
        for (j = 0; j < CHECK_IMAGE_WIDTH; j++)
        {
            ...
            Check_Image[i][j][0] = (UINT)c;
            Check_Image[i][j][1] = (UINT)c;
            Check_Image[i][j][2] = (UINT)c;
            Check_Image[i][j][3] = (UINT)c;
        }
    }
}
...
void Display()
{
    ...
    //clear render target view to a chosen Cube_vertices
    ...
    XMMATRIX translation_matrix = XMMatrixTranslation(0.0, 0.0f, 6.0f);
    ...
    //Binding the texture 
    gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpid3d11_ShaderResourceView_Texture);
    gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpid3d11_SamplerState_Texture);

    gpID3D11DeviceContext->Draw(0, 6);
...
void unInitialize()
{
    ...
    if (gpid3d11_SamplerState_Texture)
    {
        gpid3d11_SamplerState_Texture->Release();
        gpid3d11_SamplerState_Texture = NULL;
    }

    if (gpid3d11_ShaderResourceView_Texture)
    {
        gpid3d11_ShaderResourceView_Texture->Release();
        gpid3d11_ShaderResourceView_Texture = NULL;
    }
    ...
    if (gpID3D11_Cube_Buffer_VertextBuffer_Position )
    {
        gpID3D11_Cube_Buffer_VertextBuffer_Position ->Release();
        gpID3D11_Cube_Buffer_VertextBuffer_Position  = NULL;
    }

    if (gpID3D11_Cube_Buffer_VertextBuffer_Texture)
    {
        gpID3D11_Cube_Buffer_VertextBuffer_Texture->Release();
        gpID3D11_Cube_Buffer_VertextBuffer_Texture = NULL;
    }
