...
ID3D11Buffer* gpID3D11_Triangle_Buffer_VertextBuffer_Texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        szClassname,
        TEXT("Darshan's : Direct X 3D Pyramid Texture ...!"),
...
HRESULT Initialize()
{
    ...
    */
    ...
    //Copy Pyramid_vertices into above created buffer
    ...
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(Pyramid_TexCoord);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
        NULL,
        &gpID3D11_Triangle_Buffer_VertextBuffer_Texture);

    if (FAILED(hr))
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "ID3D11Device::CreateBuffer() Failed for Pyramid_vertices Buffer. \n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "ID3D11Device::CreateBuffer() Succeeded for Pyramid_vertices Buffer. \n");
        fclose(gpFile_DM);
    }

    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));

    gpID3D11DeviceContext->Map(gpID3D11_Triangle_Buffer_VertextBuffer_Texture,
        0,
        D3D11_MAP_WRITE_DISCARD,
        0,
        &mappedSubresource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11_Triangle_Buffer_VertextBuffer_Texture, 0);
...
void Display()
{
    ...
    XMMATRIX translation_matrix = XMMatrixTranslation(0.0, 0.0f, 4.0f);
    XMMATRIX rotation_matrix_y = XMMatrixRotationY(angle_Pyramid);
...
void update(void)
{
    angle_Pyramid = angle_Pyramid + 0.0005f;
...
void unInitialize()
{
    ...
    if (gpID3D11_Triangle_Buffer_VertextBuffer_Texture)
    {
        gpID3D11_Triangle_Buffer_VertextBuffer_Texture->Release();
        gpID3D11_Triangle_Buffer_VertextBuffer_Texture = NULL;
    }
