...
ID3D11VertexShader* gpID3D11VertexShader_Vertex = NULL;
ID3D11PixelShader* gpID3D11PixelShader_Vertex = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        szClassname,
        TEXT("Darshan's : Direct X 3D sphere Light Toggling...!"),
...
HRESULT Initialize()
{
    ...
    */
    ...
    const char* vertexShaderSourceCode_PS =
        ...
        "float3 light_direction : NORMAL_1;"\
    ...
    //Blog is used for to keep any data inside it
    ID3DBlob* pID3DBlob_VartexShaderCode_PS = NULL;
    ID3DBlob* pID3DBlob_Error_PS = NULL;
    ...
    //CreatevertexShader Here 
    hr = gpID3D11Device->CreateVertexShader(pID3DBlob_VartexShaderCode_PS->GetBufferPointer(),
        pID3DBlob_VartexShaderCode_PS->GetBufferSize(),
        NULL,
    ...
    const char* PixelShaderSourceCode_PS =
        ...
        "float3 ambient = (float3)la * (float3)ka;"\
        "float3 diffuse = (float3)ld * (float3)kd * max(dot(normalized_light_direction, normalized_trans_light), 0.0);\n"\

        "float3 reflection_vector = reflect(-normalized_light_direction, normalized_trans_light);\n"\
        "float3 specular = (float3)ls * (float3)ks * pow(max(dot(reflection_vector, normalized_viewer_vector), 0.0), material_shininess);\n"\

        "fong_ads_light = ambient + diffuse + specular;"\

        //"fong_ads_light = float3(1.0f, 1.0f, 1.0f);"\
        
        "float4 color = float4(fong_ads_light, 1.0);"\

        "return(color);"\
        "}";

    ID3DBlob* pID3DBlob_PixelShaderCode_PS = NULL;
    pID3DBlob_Error = NULL;
    ...
    //CreatePixelShader Here 
    hr = gpID3D11Device->CreatePixelShader(pID3DBlob_PixelShaderCode_PS->GetBufferPointer(),
        pID3DBlob_PixelShaderCode_PS->GetBufferSize(),
        NULL,
    ...
    */
    ...
    hr = gpID3D11Device->CreateInputLayout(inputElementDesc,
        _ARRAYSIZE(inputElementDesc),
        pID3DBlob_VartexShaderCode_PS->GetBufferPointer(),
        pID3DBlob_VartexShaderCode_PS->GetBufferSize(),
        &gpID3D11InputLayout_PS);
...
void Display()
{
    ...
    if (gbLight == TRUE)
    {
        ...
        gpID3D11DeviceContext->VSSetShader(gpID3D11VertexShader_Vertex, 0, 0);
        gpID3D11DeviceContext->PSSetShader(gpID3D11PixelShader_Vertex, 0, 0);
...
void unInitialize()
{
    ...
    if (gpID3D11InputLayout_PS)
    {
        gpID3D11InputLayout_PS->Release();
        gpID3D11InputLayout_PS = NULL;
    }
    ...
    if (gpID3D11PixelShader_Vertex)
    {
        gpID3D11PixelShader_Vertex->Release();
        gpID3D11PixelShader_Vertex = NULL;
    }
    if (gpID3D11VertexShader_Vertex)
    {
        gpID3D11VertexShader_Vertex->Release();
        gpID3D11VertexShader_Vertex = NULL;
    }
    if (gpID3D11RenderTargetView)
    {
        gpID3D11RenderTargetView->Release();
