...
ID3D11Buffer* gpID3D11_Cube_Buffer_VertextBuffer_Normals = NULL;
ID3D11Buffer* gpID3D11_Cube_Buffer_VertextBuffer_Texture = NULL;
ID3D11Buffer* gpID3D11_Cube_Buffer_VertextBuffer_Color = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
        szClassname,
        TEXT("Darshan's : Direct X 3D InterLeaved ...!"),
...
HRESULT Initialize()
{
    ...
    const char* vertexShaderSourceCode =
        ...
        "float4 defused_Light : NORMAL;"\
        "float2 TexCoord : TEXCOORD;"\
        "float4 color : COLOR;"\
        "};"\

        "VertexOutPut main(float4 pos : POSITION, float3 normal : NORMAL, float2 Tex : TEXCOORD, float4 col : COLOR)"\
        "{"\
        ...
        "float3 tNormal = normalize(mul((float3x3)worldViewMatrix, (float3)normal));"\
        "float3 s = (float3)normalize(lightPosition - eye_Position);"\
        "outPut.defused_Light = ld * kd * max( dot(s , tNormal), 0.0);"\
        "}"\
        
        "else"\
        "{"\
        "outPut.defused_Light = float4(1.0f, 1.0f, 1.0f, 1.0f);"\
        "}"\
    ...
    const char* PixelShaderSourceCode =
        ...
        "float4 color = input.color * input.defused_Light  * myTexture2D.Sample(mySamplerState, -input.TexCoord);"\
    ...
    */
    ...
    inputElementDesc[2].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;//glPixelStorei unpackAlling  
    ...
    //this Pyramid_vertices are taken as clock vise but openGL use anti clock vise
    ...
    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
        NULL,
        &gpID3D11_Cube_Buffer_VertextBuffer_Normals);
    ...
    gpID3D11DeviceContext->Map(gpID3D11_Cube_Buffer_VertextBuffer_Normals,
        0,
        D3D11_MAP_WRITE_DISCARD,
        0,
        &mappedSubresource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11_Cube_Buffer_VertextBuffer_Normals, 0);
    ...
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(Cube_color);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&bufferDesc,
        NULL,
        &gpID3D11_Cube_Buffer_VertextBuffer_Color);
    ...
    gpID3D11DeviceContext->Map(gpID3D11_Cube_Buffer_VertextBuffer_Color,
        0,
        D3D11_MAP_WRITE_DISCARD,
        0,
        &mappedSubresource);
    ...
    gpID3D11DeviceContext->Unmap(gpID3D11_Cube_Buffer_VertextBuffer_Color, 0);
   ...
   */
    hr = Load_D3D_Texture(L"stone.bmp", &gpid3d11_ShaderResourceView_Texture);//L == Literal before string (wide char compliand)
...
HRESULT Load_D3D_Texture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
    ...
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "DirectX::CreateWICTextureFromFile() Failed. Exiting Now...\n");
        fclose(gpFile_DM);
        return(hr);
    }
    else
    {
        fopen_s(&gpFile_DM, gszLogFileName, "a+");
        fprintf_s(gpFile_DM, "DirectX::CreateWICTextureFromFile() Succeeded...\n");
        fclose(gpFile_DM);
    }
...
void Display()
{
    ...
    XMMATRIX translation_matrix = XMMatrixTranslation(0.0f, 0.0f, 6.0f);
    
    XMMATRIX rotation_matrix_x = XMMatrixRotationX(angle_Cube);
    XMMATRIX rotation_matrix_y = XMMatrixRotationY(angle_Cube);
    XMMATRIX rotation_matrix_z = XMMatrixRotationZ(angle_Cube);
    XMMATRIX Rotation_Matrix = XMMatrixIdentity();

    Rotation_Matrix = rotation_matrix_x * rotation_matrix_y * rotation_matrix_z;

    XMMATRIX worldMatrix = Rotation_Matrix * translation_matrix;
    ...
    if (gbLight == TRUE)
    {
        ...
        constantBuffer.KD = XMVectorSet(0.85f, 0.85f, 0.85f, 0.85f);
