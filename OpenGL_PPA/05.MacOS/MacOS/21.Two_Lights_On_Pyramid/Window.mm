...
-(void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    defer:NO];
    [window setTitle: @"DVM: MacOS Pyramid with Two Lights."];
    [window center];
...
/*splite 1.model view, 2.projection matrics */
    ...
    GLuint LA_Uniform_DM[2];/*Deffuse*/
    GLuint LD_Uniform_DM[2];/*Deffuse*/
    GLuint LS_Uniform_DM[2];/*Deffuse*/
    GLuint LPosition_Uniform_DM[2];/*Positon*/

    GLuint KA_Uniform_DM;/*Material*/
    GLuint KD_Uniform_DM;/*Material*/
    GLuint KS_Uniform_DM;/*Material*/
    ...
    -(void) prepareOpenGL
    {
        ...
        const GLchar* vertexShaderSourceCode =
            ...
            "vec3 view_vector = normalize(vec3(u_light_position[0] - eye_cordinates));\n" \
            ...
            "reflection_vector[0] = reflect(-light_direction[0], tnormal);\n" \
            "ambient[0] = u_la[0] * u_ka;\n" \
            "diffuse[0] = u_ld[0] * u_kd * max(dot(light_direction[0], tnormal), 0.0);\n" \
            "specular[0] = u_ls[0] * u_ks * pow( max(dot(reflection_vector[0], view_vector), 0.0), u_shininess);\n" \
            "fong_ads_light = ambient[0] + diffuse[0] + specular[0];\n" \
            

            "light_direction[1] = normalize(vec3(u_light_position[1] - eye_cordinates));\n" \
            "reflection_vector[1] = reflect(-light_direction[1], tnormal);\n" \
            "ambient[1] = u_la[1] * u_ka;\n" \
            "diffuse[1] = u_ld[1] * u_kd * max(dot(light_direction[1], tnormal), 0.0);\n" \
            "specular[1] = u_ls[1] * u_ks * pow( max(dot(reflection_vector[1], view_vector), 0.0), u_shininess);\n" \
            "fong_ads_light = fong_ads_light + ambient[1] + diffuse[1] + specular[1];\n" \
    ...
    -(void) drawView
    {
        ...
        if (bLights)
        {
            ...
            glUniform3fv(LD_Uniform_DM[0], 1, Lights[0].lAmbient);
            glUniform3fv(LD_Uniform_DM[0], 1, Lights[0].lDefues);
            glUniform3fv(LD_Uniform_DM[0], 1, Lights[0].lSpecular);
            glUniform4fv(LPosition_Uniform_DM[0], 1, Lights[0].lPosition);
            
            glUniform3fv(LD_Uniform_DM[1], 1, Lights[1].lAmbient);
            glUniform3fv(LD_Uniform_DM[1], 1, Lights[1].lDefues);
            glUniform3fv(LD_Uniform_DM[1], 1, Lights[1].lSpecular);
